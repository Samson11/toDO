...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'todo-mac.zip',
    path: path.join(outPath, 'toDO-darwin-x64', 'toDO.app')
  }, {
    name: 'todo-windows.zip',
    path: path.join(outPath, 'toDO-win32-ia32')
  }, {
    name: 'todo-linux.zip',
    path: path.join(outPath, 'toDO-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'toDOSetup.exe',
      path: path.join(outPath, 'windows-installer', 'toDO.exe')
    }, {
      name: `toDO-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `toDO-${version}-full.nupkg`)
    }])
  })
}
